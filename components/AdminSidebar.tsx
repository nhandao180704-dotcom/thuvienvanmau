...
import { 
  ...
  MessageCircle    // Thêm icon MessageCircle
} from 'lucide-react'
...
export default function AdminSidebar() {
  ...
  // Đã bổ sung 2 module mới: Quản lý học sinh & Quản lý bình luận
  const menuItems = [
    ...
    { label: 'Quản lý học sinh', href: '/admin/users', icon: Users, isActive: pathname.startsWith('/admin/users') },
    { label: 'Quản lý bình luận', href: '/admin/comments', icon: MessageCircle, isActive: pathname.startsWith('/admin/comments') },
  ...
  return (
    <>
      ...
      >
        ...
        >
          <div className="w-10 h-10 rounded-xl bg-[#0052CC] flex items-center justify-center shadow-md shrink-0 group-hover:scale-105 transition-transform">
          ...
          <div className="flex flex-col">
            ...
            <p className="text-sm font-medium text-slate-500 leading-tight mt-0.5 uppercase tracking-wide">Admin</p>
          </div>
        </Link>
          ...
          {menuItems.map((item, idx) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                onClick={() => setIsOpen(false)}
                className={`group relative flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-all duration-200 overflow-hidden ${
                  item.isActive
                    ? 'bg-[#F0F5FF] text-[#0052CC] ring-1 ring-blue-100/50 shadow-sm' 
                    : 'text-slate-600 hover:bg-slate-50 hover:text-[#0052CC]'
                }`}
              >
                {/* Đường viền xanh bo tròn góc bên trái cho Tab đang chọn, thêm hiệu ứng sáng */}
                {item.isActive && (
                  <div className="absolute left-0 top-1/2 -translate-y-1/2 h-8 w-1.5 bg-[#0052CC] rounded-r-full shadow-[0_0_8px_rgba(0,82,204,0.4)]" />
                )}
                
                {/* Thêm hiệu ứng phóng to icon khi hover */}
                <Icon 
                  size={20} 
                  className={`transition-transform duration-200 ${
                    item.isActive 
                      ? 'text-[#0052CC]' 
                      : 'text-slate-400 group-hover:scale-110 group-hover:text-[#0052CC]'
                  }`} 
                />
