...
function QuizForm() {
  ...
  // Cấu hình chia điểm (dành cho đề hỗn hợp)
  const [mcSplit, setMcSplit] = useState(7) // Mặc định Trắc nghiệm 7đ, Tự luận 3đ
  ...
  const [questions, setQuestions] = useState<any[]>([
    {
      ...
      content: '',
      points: 1, // Sẽ bị ghi đè bởi hệ thống đối với câu trắc nghiệm
      options: ['', '', '', ''],
      correct_answer: '0' 
    }
  ])

  // --- LOGIC TÍNH ĐIỂM TỰ ĐỘNG THANG 10 ---
  ...
  // Điểm của mỗi câu trắc nghiệm (chia đều)
  const pointPerMc = mcCount > 0 ? (targetMcScore / mcCount) : 0
  
  // Tổng điểm tự luận người dùng đã nhập
  ...
  const addQuestion = (type: 'multiple_choice' | 'essay') => {
    setQuestions([
      ...questions,
      {
        ...
        type: type,
        content: '',
        points: type === 'essay' ? 1 : 0, // Mặc định tự luận 1đ, trắc nghiệm hệ thống tự lo
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // KIỂM TRA ĐIỀU KIỆN ĐIỂM SỐ TRƯỚC KHI LƯU
    if (essayCount > 0 && Math.abs(currentEssaySum - targetEssayScore) > 0.01) {
      showError(`Tổng điểm phần Tự luận phải bằng đúng ${targetEssayScore} điểm. (Hiện tại bạn phân bổ được ${currentEssaySum} điểm)`)
      return
    }
    ...
    try {
      const { data: quizData, error: quizErr } = await supabase
        .from('quizzes')
        .insert([{ title, grade, duration, is_published: true }])
        .select()
        .single()
      ...
      const questionsToInsert = questions.map((q, index) => ({
        ...
        content: q.content,
        // Ép điểm trắc nghiệm theo tính toán tự động
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="fixed top-16 right-0 left-0 md:left-64 bottom-0 overflow-y-auto p-4 md:p-8 max-w-5xl mx-auto">
        ...
        <h1 className="text-2xl font-black text-slate-900 mb-6">Tạo Đề Thi (Thang Điểm 10)</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 pb-20">
          ...
          <div className="bg-indigo-50 border border-indigo-100 p-6 rounded-2xl shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* Tùy chỉnh điểm chia tỷ lệ nếu là đề hỗn hợp */}
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center bg-emerald-50 p-3 rounded-lg border border-emerald-100">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-xs text-emerald-500 mt-0.5">Nhập thủ công cho từng câu</p>
                  </div>
                </div>
              </div>
            </div>
                ...
                <div className="space-y-4">
                  ...
                  {q.type === 'multiple_choice' && (
                    <>
                      ...
                      <div className="mt-4 pt-4 border-t border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
                        ...
                        {/* HỆ THỐNG TỰ ĐỘNG BÁO ĐIỂM Ở ĐÂY, KHÔNG CHO NHẬP */}
                  ...
                  {q.type === 'essay' && (
                    <div className="mt-4">
                      ...
                      {/* TỰ LUẬN ĐƯỢC NHẬP ĐIỂM THỦ CÔNG */}
                      <div className="mt-4 bg-emerald-50 p-4 rounded-xl border border-emerald-100 flex items-center gap-4">
                        ...
                        <span className="text-xs text-emerald-600 italic">* Hãy đảm bảo tổng điểm các câu Tự luận khớp với bảng thống kê bên trên.</span>
                      </div>
                    </div>
                  )}
                </div>
